    ...
    if (!(msg.channel instanceof Discord.DMChannel)) {
      ...
      try {
        ...
        if (text[0].includes("m")) {
          ...
          if (!(duration_arr[1] === ""))
            var duration_res =
              parseInt(duration_arr[0]) * 60 * 1000 +
              parseInt(duration_arr[1]) * 1000;
          else var duration_res = parseInt(duration_arr[0]) * 60 * 1000;
        } else var duration_res = parseInt(text[0]) * 1000;
        ...
        msg.channel
          .send(
            "***" +
              text +
              "***\nDurée du sondage: " +
              Singleton.millisToMinutesAndSeconds(duration) +
              "\nAuteur: <@" +
              msg.author.id +
              ">"
          )
        ...
        msg.reply(
          "Syntaxe invalide, exemple d'utilisation: `" +
            Singleton.prefix +
            "vote 5m30 Aimez-vous la couleur rouge ?`, pour plus de détails, faire la commande `" +
            Singleton.prefix +
            "help`"
        );
      }
